...
import pandas as pd
from datetime import datetime # V70 新增：用於顯示更新時間

# --- CSS: V70 排版微調 (維持 V69 的高密度與清晰度) ---
...
def render_header(title, show_monitor=False):
    ...
    if show_monitor:
        ...
        if is_live:
            ...
            st.caption("資料來源: Yahoo Finance / TWSE | V70 即時監控版")
...
def render_kline_pattern_card(title, pattern_data):
    ...
    with st.container(border=True):
        ...
        with c1:
            ...
            fig.update_layout(
                margin=dict(l=2, r=2, t=10, b=2), height=180,
...
def generate_trade_advice(price, high, low, m5, m20, m60, rsi, strategy_type="general"):
    ...
    if strategy_type == 'day': 
        stop_price = low * 0.99; target_price = high * 1.02
        hold_time = "當日沖銷"
        ...
        stop_price = m20; target_price = price * 1.08
        hold_time = "3-5 天"
        ...
        stop_price = m60; target_price = price * 1.20
        hold_time = "1-3 個月"
        if price > m60:
        ...
        stop_price = m20; target_price = price * 1.05
        hold_time = "視情況"
        if price > m20: 
