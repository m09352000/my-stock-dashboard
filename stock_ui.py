...
def render_back_button(callback_func):
    ...
    if c2.button("⬅️ 返回列表", use_container_width=True): callback_func()

def render_term_card(title, content):
    with st.container(border=True):
        st.subheader(f"📌 {title}"); st.markdown("<hr class='compact'>", unsafe_allow_html=True)
        st.markdown(f"<div class='term-content'>{content}</div>", unsafe_allow_html=True)

def render_kline_pattern_card(title, pattern_data):
    morph = pattern_data.get('morphology', '無資料'); psycho = pattern_data.get('psychology', '無資料')
    action_html = pattern_data.get('action', '無資料'); raw_data = pattern_data.get('data', [])
    with st.container(border=True):
        ...
        with c2:
            st.markdown(f"### 💡 {title}"); st.markdown("<hr class='compact'>", unsafe_allow_html=True)
            st.caption(morph); st.caption(psycho); st.markdown(f"<div class='action-list'>{action_html}</div>", unsafe_allow_html=True)

def render_company_profile(summary):
    if summary and summary != "暫無詳細描述":
        with st.expander("🏢 公司簡介與業務", expanded=False): st.write(summary)

def render_metrics_dashboard(curr, chg, pct, high, low, amp, main_force, vol, vol_yest, vol_avg, vol_status, foreign_held, turnover_rate, bid_ask_data, color_settings, realtime_data=None):
    is_realtime = False
    if realtime_data:
        is_realtime = True
        curr = realtime_data['latest_trade_price']; high = realtime_data['high']; low = realtime_data['low']
        vol = int(float(realtime_data['accumulate_trade_volume']))
        prev_close = realtime_data['previous_close']
        if prev_close > 0:
            chg = curr - prev_close; pct = (chg / prev_close) * 100; amp = ((high - low) / prev_close) * 100
        if chg > 0: val_color = "#FF2B2B"
        elif chg < 0: val_color = "#00E050"
        else: val_color = "#FFFFFF"
    else: val_color = "white"
    ...
    with st.container():
        ...
        m1.markdown(f"<div style='font-size:0.9rem; color:#d0d0d0'>成交價 {live_indicator}</div><div style='font-size:1.6rem; font-weight:800; color:{val_color}; line-height:1.2'>{curr:.2f} <span style='font-size:1rem'>({chg:+.2f} / {pct:+.2f}%)</span></div>", unsafe_allow_html=True)
        m2.metric("最高價", f"{high:.2f}"); m3.metric("最低價", f"{low:.2f}"); m4.metric("振幅", f"{amp:.2f}%"); m5.metric("主力動向", main_force)
        v1, v2, v3, v4, v5 = st.columns(5)
        v1.metric("今日量", f"{int(vol):,} 張")
        t_label = "🔥 過熱" if turnover_rate > 20 else ("熱絡" if turnover_rate > 10 else ("❄️ 冷門" if turnover_rate < 0.5 else "正常"))
        v2.metric("週轉率", f"{turnover_rate:.2f}%", t_label)
        v3.metric("五日均量", f"{int(vol_avg/1000):,} 張")
        v4.metric("量能狀態", vol_status); v5.metric("外資持股", f"{foreign_held:.1f}%")
    
    if bid_ask_data:
        st.markdown("---"); st.caption("📊 即時五檔 (Best Bid/Ask)")
        b_price = bid_ask_data.get('bid_price', ['-'])[0]; b_vol = bid_ask_data.get('bid_volume', ['-'])[0]
        a_price = bid_ask_data.get('ask_price', ['-'])[0]; a_vol = bid_ask_data.get('ask_volume', ['-'])[0]
        c1, c2 = st.columns(2)
...
def generate_trade_advice(price, high, low, m5, m20, m60, rsi, strategy_type="general"):
    ...
    if strategy_type == 'day': 
        if price > m5 and price > pivot: return "🔥 強力作多", "#FF2B2B", high * 1.02, low * 0.99, f"{pivot:.1f} 附近", f"跌破 {m5:.1f}", "當日沖銷", "量價齊揚站上樞紐，多方動能強勁。"
        elif price < pivot: return "🧊 偏空操作", "#00E050", high * 1.02, low * 0.99, f"反彈 {pivot:.1f} 不過", "急殺出量", "當日沖銷", "股價受制於樞紐之下，賣壓重。"
        else: return "⚖️ 區間震盪", "#FF9F1C", high * 1.02, low * 0.99, f"{s1:.1f} 支撐", f"{r1:.1f} 壓力", "當日沖銷", "多空膠著，建議區間操作。"
    elif strategy_type == 'short':
        if price > m5 and m5 > m20: return "🚀 穩健買進", "#FF2B2B", price * 1.08, m20, f"回測 {m5:.1f}", f"跌破 {m20:.1f}", "3-5 天", "均線多頭排列，短線趨勢向上。"
        elif price < m5: return "📉 等待止穩", "#FF9F1C", price * 1.08, m20, f"接近 {m20:.1f}", "有效跌破月線", "3-5 天", "短線乖離修正，等待回測月線。"
    elif strategy_type == 'long':
        if price > m60: return "🐢 長線續抱", "#FF2B2B", price * 1.20, m60, f"{m60:.1f} 附近", "季線下彎", "1-3 個月", "股價站穩生命線，長線保護短線。"
        else: return "⏳ 觀望", "#aaaaaa", price * 1.20, m60, "突破季線", "續破底", "1-3 個月", "處於空頭或整理架構。"
    else: 
        if price > m20: return "💪 強勢持有", "#FF2B2B", price * 1.05, m20, "量縮不破低", "爆量收黑", "視情況", "趨勢向上，轉弱即跑。"
        else: return "⚠️ 轉弱減碼", "#00E050", price * 1.05, m20, "暫不建議", f"反彈 {m20:.1f}", "視情況", "籌碼鬆動轉弱。"
    return "觀望", "#aaaaaa", 0.0, 0.0, "-", "-", "-", "數據盤整中"

def render_detailed_card(code, name, price, df, source_type="yahoo", key_prefix="btn", rank=None, strategy_info=None):
    chg_color = "black"; pct_txt = ""; action_title = "計算中"; action_color_hex = "#aaaaaa"
    target_val = 0.0; stop_val = 0.0; entry_txt = "-"; exit_txt = "-"; hold_txt = "-"; reason_txt = "資料不足"
    strat_type = "general"
    if strategy_info:
    ...
    if df is not None and not df.empty:
        try:
            ...
            if len(df) > 20:
                m5 = df['Close'].rolling(5).mean().iloc[-1]; m20 = df['Close'].rolling(20).mean().iloc[-1]
                m60 = df['Close'].rolling(60).mean().iloc[-1]
                delta = df['Close'].diff(); u = delta.copy(); d = delta.copy(); u[u<0]=0; d[d>0]=0
                rs = u.rolling(14).mean() / d.abs().rolling(14).mean()
                rsi = (100 - 100/(1+rs)).iloc[-1] if not rs.isna().iloc[-1] else 50
                action_title, action_color_hex, target_val, stop_val, entry_txt, exit_txt, hold_txt, reason_txt = generate_trade_advice(curr, high, low, m5, m20, m60, rsi, strat_type)
        except: pass
...
def render_ai_report(curr, m5, m20, m60, rsi, bias, high, low, df=None):
    ...
    pivot = (high + low + curr) / 3; r1 = 2 * pivot - low; s1 = 2 * pivot - high
    ...
    with t1:
        ...
        with c1:
            ...
            elif curr > m20: st.warning("🌤️ **震盪偏多**")
            else: st.info("🌧️ **震盪偏空**")
        with c2:
            st.markdown("#### ⚡ 動能指標")
    ...
    with t2:
        ...
        cp1.metric("壓力 (R1)", f"{r1:.2f}"); cp2.metric("中軸 (P)", f"{pivot:.2f}"); cp3.metric("支撐 (S1)", f"{s1:.2f}")
    with t3:
        if df is not None and len(df) >= 5:
            try:
                ...
                if is_red(c3) and is_red(c2) and is_red(c1) and c1['Close']>c2['Close']>c3['Close']:
                    title = "💂‍♂️ 紅三兵"; box = "bull-box"; advice = "多頭部隊集結完畢，趨勢由空翻多。"
