# ui_components.py
# V115: 視覺層 (加入翻譯與防呆 Key)

import streamlit as st
import plotly.graph_objects as go
from plotly.subplots import make_subplots
import ui_styles
import logic_database as db # 引入 DB 以使用翻譯功能
...
def render_fundamental_panel(stock_info):
    ...
    summary_en = stock_info.get('longBusinessSummary', '')
    
    # 這裡進行翻譯
    summary_zh = db.translate_text(summary_en)
    ...
    with st.container(border=True):
        st.markdown(f"### 🏢 {name} 企業概況")
        c1, c2 = st.columns([3, 1])
        with c1:
            st.caption(f"產業分類: {sector}")
            with st.expander("📖 點擊查看詳細介紹 (中文)", expanded=True):
                st.write(summary_zh)
        with c2:
...
def render_chart(df, title, color_settings, key):
    # 強制檢查 Key，防止 DuplicateElementId
...
# ... (其餘函式如 render_ai_battle_dashboard 保持不變) ...
# 請務必保留原有的 render_ai_battle_dashboard, render_detailed_card 等
