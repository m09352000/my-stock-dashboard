...
def render_fundamental_panel(stock_info):
    name = stock_info.get('name', stock_info.get('code', '未知個股'))
    ...
    # 翻譯
    summary_zh = db.translate_text(summary)

    with st.container(border=True):
        c_main, c_info = st.columns([3, 1])
        with c_main:
            st.markdown(f"### 🏢 {name} 基本面透視")
            st.caption(f"板塊: {sector} | 產業: {industry}")
            with st.expander("📖 查看業務介紹 (中文)", expanded=False):
                st.write(summary_zh)
        with c_info:
