...
def render_chart(df, title, color_settings, key=None):
    ...
    if 'MA5' in df.columns: fig.add_trace(go.Scatter(x=df.index, y=df['MA5'], line=dict(color='#FF00FF', width=1), name='5MA'), row=1, col=1)
    if 'MA20' in df.columns: fig.add_trace(go.Scatter(x=df.index, y=df['MA20'], line=dict(color='#FFA500', width=1), name='20MA'), row=1, col=1)
...
def render_detailed_card(code, name, price, df, src, key_prefix, rank, strategy_info, score, w_prob):
    with st.container(border=True):
        ...
        with c1: st.markdown(f"<div class='rank-badge rank-{rank if rank<=3 else 'norm'}'>{rank}</div>", unsafe_allow_html=True)
        with c2:
            st.markdown(f"### {name}")
            st.caption(f"{code} | ${price:.2f}")
            if src == "github_voidful": st.caption("來源: GitHub")
        with c3: st.info(f"🎯 {strategy_info}")
        with c4:
        ...
        with c5:
            st.write("")
            if st.button("查看", key=f"{key_prefix}_btn_{code}", use_container_width=True): return True
    return False
